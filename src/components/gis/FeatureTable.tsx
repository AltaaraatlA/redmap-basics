...
import { Trash2, MapPin, Spline, Hexagon, Type as TypeIcon, ChevronDown, ChevronUp } from "lucide-react";
...
export function FeatureTable() {
  ...
  return (
    <div
      className={cn(
        "pointer-events-auto flex flex-col overflow-hidden border-t border-border bg-card/95 backdrop-blur transition-[height]",
        open ? "h-72" : "h-10"
      )}
    >
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex h-10 shrink-0 items-center justify-between border-b border-border bg-card px-4 text-left"
      >
